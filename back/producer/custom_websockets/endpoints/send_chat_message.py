    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(
                ...
                json={"chat": chat, "sender": sender, "text": text, "tag": tag},
                timeout=5.0,
            )
...
async def send_message_to_chat(websocket: WebSocket, chat: str):
    ...
    logger.info(f"/send-message/chat/{chat} connected")
    try:
        while True:
            raw_send_message_json = await websocket.receive_text()
            logger.info(f"Received raw message: {raw_send_message_json}")

            send_message_request = parse_send_chat_message_request(
                raw_send_message_json
            )
        ...
        logger.info(f"Client disconnected from chat {chat}")
    except Exception as e:
...
def parse_send_chat_message_request(raw_data: str):
    ...
    return SendChatMessageRequest(
        sender=data["sender"], text=data["text"], tag=data["tag"]
    )
